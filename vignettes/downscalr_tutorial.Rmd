...
Before we can start with the actual downscaling, some required packages have to be mounted to your `R session`. The first three are standard packages to facilitate data manipulation and the last one, of course, is the subject of this tutorial.
...
#plot function
LU_plot <- function(rasterfile, results, start.values, year=NULL, LU=NULL, color = "Greens", label = "Area in ha per pixel"){
  
  save_geo_vals <- data.frame(getValues(rasterfile)) #just the SimU IDs
  ...
  if(is.null(year) & is.null(LU)){
    inputs <- results %>% group_by(ns, lu.to, times) %>% summarise(value = sum(value),.groups = "keep")
  } else if(!(is.null(year) | is.null(LU))){
    inputs <- results %>% group_by(ns, lu.to, times) %>% summarise(value = sum(value),.groups = "keep") %>% subset(lu.to==LU & times==year)
  } else if(is.null(year)){
    inputs <- results %>% group_by(ns, lu.to, times) %>% summarise(value = sum(value),.groups = "keep") %>% subset(lu.to==LU)
  } else {
    inputs <- results %>% group_by(ns, lu.to, times) %>% summarise(value = sum(value),.groups = "keep") %>% subset(times==year)
  }
  
  plot_df <- merge(plot_df, inputs, by="ns")

  ggplot() +  
    geom_tile(data=plot_df, aes(x=x, y=y, fill=value, group=lu.to), alpha=0.8) + 
    scale_fill_distiller(palette = color , name=label, direction = 1)+
...
#Compare LUC in 2010 to intial LU allocation in 2000
results <- results_DS$out.res
initial_LU <- argentina_df$lu_levels
initial_LU$lu.to <- initial_LU$lu.from
initial_LU$times <- 2000
results_with_init <- rbind(initial_LU,results)

LU_plot(argentina_raster, results_with_init, color="Reds")

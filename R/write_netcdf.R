...
#' 
write_netcdf <- function(res, rasterfile, filename = "nc_v1.nc", raster.crs="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0", times=NULL, ns=NULL, lu.to=NULL, value=NULL, label=NULL, .=NULL){
  ...
  lon <- seq(raster::extent(geosims)[1]+(raster::res(geosims)[2]/2),raster::extent(geosims)[2]-(raster::res(geosims)[2]/2),length.out=dim(geosims)[2])
  lat <- seq(raster::extent(geosims)[4]-(raster::res(geosims)[1]/2),raster::extent(geosims)[3]+(raster::res(geosims)[1]/2),length.out=dim(geosims)[1])
  ...
  dim_lc_class <- ncdf4::ncdim_def("lc_class", paste(paste0(seq(1,length(lc_class),1),"=",lc_class),collapse ="/"),seq(1,length(lc_class),1))
  dim_area <- ncdf4::ncdim_def("area", paste0("land_area"),1)
  
  # create variables
  fillvalue <- NA
  LandCover_pixshare_long <- 'share of pixel occupied by various land covers'
  LandCover_pixshare <- ncdf4::ncvar_def(name = "LC_area_share",units = "share of pixel area",
  ...
  PixelArea <- ncdf4::ncvar_def(name = paste0('Land_area'),units = "as input (km2)",
  ...
  ncid_out <- ncdf4::nc_create(filename, list(LandCover_pixshare,PixelArea))
  ...
  res_file_LU <- res %>% 
    ...
    rename(label=lu.to) %>% 
    pivot_wider(id_cols = c(times, ns), names_from = "label", values_from = "value") %>% 
    ungroup() %>% 
  ...
  res_file_LU <- res_file_LU %>% 
    ...
    pivot_longer(cols=!c(ns,times) ,names_to = "label", values_to = "value") %>% 
    group_by(times,ns,label) %>%
    arrange(label) %>% 
    pivot_wider(id_cols = ns, names_from = c(times, label), values_from = value) %>% 
    left_join(ns_area) %>% 
  ...
  for (t in 1:length(time)) {
    for (c in 1:length(lc_class)) {
      ...
      ncdf4::ncvar_put(ncid_out, LandCover_pixshare, t(data), start=c(1, 1, c, t), 
  ...
  ncdf4::ncvar_put(ncid_out,PixelArea,t(data), start=c(1, 1, 1), 
